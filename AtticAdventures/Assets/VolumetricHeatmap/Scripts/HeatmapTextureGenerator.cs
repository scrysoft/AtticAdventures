...
namespace HeatmapVisualization
{
	...
	public class HeatmapTextureGenerator
	{
		...
		public float[] CalculateHeatTexture(Vector3[] points, Settings settings)
		{
			...
			if (settings.gaussStandardDeviation > 0)
			{
		...
		{
			return Mathf.Exp(-0.5f * Mathf.Pow(distance / gaussStandardDeviation, 2.0f)) / (gaussStandardDeviation * Mathf.Sqrt(2.0f * Mathf.PI));
		}
		#endregion
	}
}
